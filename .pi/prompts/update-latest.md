...
Use `github_get_file_contents` to fetch `CHANGELOG.md` from the template repo:
```
...
`github_get_file_contents` and show a comparison:
...
- GitHub: `github_*` (all operations)
- Redis: `mcp__redis-*__*` (all operations)
- Context7: `Context7_*` (all operations)
