...
for rule in ptu:
    if rule.action == 'block' and rule.enabled:
        ...
        if rule.name == 'mcp-server-guard':
            test_input['tool_name'] = 'mcp__supabase-testproject__execute_sql'
        elif 'n8n' in rule.name and 'delete' in rule.name:
            test_input['tool_name'] = 'mcp__n8n-mcp-test__n8n_delete_workflow'
            test_input['tool_input'] = {'id': 'test'}
        elif rule.name == 'playwright-full-page':
            test_input['tool_name'] = 'mcp__playwright__browser_take_screenshot'
            test_input['tool_input'] = {'fullPage': 'true'}
        elif rule.name == 'n8n-use-essentials':
            test_input['tool_name'] = 'mcp__n8n-mcp-test__get_node_info'
        elif rule.name == 'n8n-fetch-blocker':
            test_input['tool_name'] = 'mcp__n8n-mcp-test__n8n_get_workflow'
            test_input['tool_input'] = {'mode': 'full', 'id': 'test'}
        elif rule.name == 'n8n-executions-full':
            test_input['tool_name'] = 'mcp__n8n-mcp-test__n8n_executions'
...
for rule in ptu:
    if rule.action == 'warn' and rule.enabled:
        ...
        if 'destructive-sql' in rule.name:
            test_input['tool_name'] = 'mcp__supabase-test__execute_sql'
            test_input['tool_input'] = {'query': 'DROP TABLE test'}
        elif 'migration-safety' in rule.name:
            test_input['tool_name'] = 'mcp__supabase-test__apply_migration'
            ...
            test_input['tool_name'] = 'mcp__github__get_file_contents'
...
# Engine wildcards
print("\n=== Engine Capabilities ===")
check("Wildcard matching", engine._matches_tool('mcp__n8n-mcp-*__*', 'mcp__n8n-mcp-test__n8n_get_workflow'), True)
