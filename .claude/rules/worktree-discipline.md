...
**Origin (2026-05-18)**: the operator — *"i am sick of this weird worktree stuff i never
...
**One project folder: `~/code/<your-project>`.** It is the canonical clone. Every job is a
...
2. **Never `/tmp/*`** — macOS tmpfs triggers git auto-lock; checkout hangs silently
   mid-populate.
3. The canonical clone lives at `~/code/<your-project>`. If it is not there, clone it there.
...
- **Exception worktree** on a branch with an identical `package-lock.json`: symlink is
  instant — `ln -sfn ~/code/<your-project>/node_modules <worktree>/node_modules`.
...
(the single-Mac norm — see `multi-session-arc-coordination.md`) never
...
```bash
git fetch origin main
git worktree add ~/code/<project>-<slug> origin/main
cd ~/code/<project>-<slug> && git switch -c feat/<slug>
ln -sfn ~/code/<your-project>/node_modules ./node_modules   # only if tests/build needed
```
...
```bash
cleanup() {
  rm -f ~/code/<project>-<slug>/node_modules
  git worktree remove --force ~/code/<project>-<slug> 2>/dev/null
  git worktree prune
}
...
- `.claude/hooks/worktree-guard.sh` — PreToolUse Bash hook. **Structurally ENFORCES
  ...
  **Clean-worktree false-positive**: the dirty-tree `git switch`/`checkout` guard reads the
  SHARED git state (the one `.git` every worktree shares), so it FALSE-POSITIVES on a
  freshly-created CLEAN worktree when the *main clone* (or any sibling) has unrelated
  uncommitted files. Symptom: a "branch-switch on a dirty tree" block immediately after
  `git worktree add` + `git switch -c`, even though the new worktree is empty. Recovery:
  confirm the new worktree itself is clean (`git -C <worktree> status --short` → empty),
  then re-run the switch with the `GIT_SWITCH_FORCE_DIRTY=1` prefix in the command STRING.
  The override is safe here precisely because the fresh worktree has nothing to lose — the
  "dirty" files live in a different working tree the switch never touches.
