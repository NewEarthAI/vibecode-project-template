...
description: |
  The in-repo n8n + config topology emitter — the fourth M3 mapper. Reads your project's
...
> my-project GitHub repo perfectly mapped" (build-plan §5 row 1, §6 line 146) is DONE — only
...
A Claude Code skill that reads your project's **in-repo n8n workflow JSON files +
...
> **One `bulk-write`, not a `write-node` loop.** At my-project v1 scale (4 workflows, ~78
...
Comps" with 33 nodes, while the LIVE cloud workflow with the same id is now named "your project -
...
1. Confirm the receiving repo's in-repo n8n directories (my-project uses `workflows/` +
...
4. **your project has no n8n** (uses Make.com); on your project the in-repo n8n part emits nothing — the
