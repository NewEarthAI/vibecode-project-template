...
|---|---|---|
| **alias** | starts with `<alias>/` where `<alias>` is a key in `alias_map` (BuyBox-AI: `@`) AND the char after the alias is `/` | resolve `<alias>` → its dir, extension-resolve, emit `imports` edge |
...
`source == <alias>` (bare) OR `source` starts with `<alias> + "/"`. For BuyBox-AI's
...
## Worked examples (from the live BuyBox-AI smoke test)
