...
name: code-emitter
description: |
  The in-repo code topology emitter — the third M3 mapper. Reads your project's
...
A Claude Code skill that reads your project's **in-repo TypeScript / TSX source
...
The within-repo edge class is `imports`; the cross-system layer (below) additionally produces
`reads_from` / `writes_to` / `invokes` / `calls` edges to Supabase + external targets.

### Cross-system edges (visual layer — Phase 1a + follow-up #1)

One tree-sitter walk per file scans two call families and emits cross-system edges + `external_endpoint`
nodes. These live ONLY in `.edges` (a separate render layer) — they do NOT enter `depends_on` or the
drift blast walk (R5).

| call family | record field | target | confidence |
|---|---|---|---|
| supabase `X.from('t')` / `X.functions.invoke('f')` (Phase 1a; R4-gated to a bound client) | `supabase_calls` | the resolved `table` / `edge_function` node | declared-medium / -high |
| bare global `fetch('<url>', {method})` (follow-up #1) | `fetch_calls` | a literal external URL → an `external-api` `external_endpoint`; a literal Supabase REST/fn URL → the resolved `table`/`edge_function` | declared-high / -medium |

Honesty boundaries (LOCKED): a **dynamic** target (`from(variable)`, `fetch(url)`, a template-with-
substitution, a no-arg call) OR an **unparseable** static URL OR an unresolved literal under non-`covered`
coverage → a **`blind-spot`** `external_endpoint` (amber, dashed) — **never dropped, never green**. An
unresolved literal under definitively-`covered` supabase coverage → a **counted drop** (a genuine dangling
ref). fetch `external_endpoint` nodes carry emitter `dependency_cruiser` (the joint-attribution exception —
detection lives here because only this emitter parses the TypeScript source). The fetch URL classifier
(`classify_fetch_url`) is pinned identical to the n8n `classify_url` by the `fetch-external.sh` parity eval.
...
- **`ts_module.is_entry`**: `true` ONLY for `repo:src/main.tsx` (my-project's known Vite
...
5. **Assemble canonical-shape nodes + edges via the frozen jq transform**:
   ...
   my-project it is `{"@":"src"}` (the `@/*` → `./src/*` mapping). See
...
7. **Print the completeness diagnostic** (the operator's safety net): the transform
   ...
   The cross-system layer adds these counters — the **supabase** pass (Phase 1a):
   `supabase_calls_total`, `cross_system_edges_declared_high`, `cross_system_edges_declared_medium`,
   `cross_system_blind_spots`, `cross_system_counted_drops`, `supabase_coverage_seen`; and the
   **fetch** pass (follow-up #1): `fetch_calls_total`, `fetch_external_endpoints`,
   `fetch_edges_declared_high`, `fetch_edges_declared_medium`, `fetch_blind_spots`,
   `fetch_counted_drops`. A non-zero `*_blind_spots` means ≥1 cross-system target is runtime-constructed
   or unresolvable (rendered amber — investigate if any were expected to be static, resolvable calls).
   A non-zero `*_counted_drops` (only possible when supabase-live coverage is `covered`) is a genuine
   dangling reference (a call to a table/function that does not exist in the substrate).

> **One `bulk-write`, not a `write-node` loop.** At my-project v1 scale (~831 ts_module +
...
| Multi-language (Python edge functions etc.) | NOT emitted v1 | UA supports 9 languages; my-project edge functions are Deno TS only; v1.1 if entities adopt others |

## Propagation — how to rebind to a non-my-project entity

When `/push-to-template` propagates this skill and another entity (your project, your project)
...
2. **Confirm the tsconfig alias map**. The `--argjson alias_map` passed to the transform
   must match the receiving repo's `tsconfig.json` `paths`. my-project is `{"@":"src"}`;
...
per-argument. At my-project v1 scale the substrate's re-derived `depends_on` /
...
### Extract-time cost — the cross-system scan skip-gate (follow-up #3)

`extract.mjs` runs UA's `analyzeFile` (which parses internally) and then a SECOND tree-sitter
parse for the cross-system call scan (`extractCrossSystemCalls`). UA does not expose its parse
tree, so a single shared parse would require forking the UA library — out of scope. Instead the
second parse is **skipped** for any file that provably contains neither call family: no Supabase
client import (the R4-gated supabase branch can't fire) AND no bare `fetch(` token (the
whitespace/newline-tolerant `\bfetch\s*\(` test; a member `x.fetch()` is not a scan target). The
gate only ever permits a SKIP when the token is absent — there is no false-negative path (proven
by `evals/fetch-external.sh` Part 1b), and a false-positive match merely does the parse anyway.
On a typical app the majority of files (neither Supabase-importing nor fetch-using) skip the
redundant parse.
...
1. **`is_entry` is a single hard-coded value.** Only `repo:src/main.tsx` is marked
   ...
   shortcut for my-project's known Vite entry. A non-your project entity with a different entry
...
3. **Cross-system edges are inferred for two call families; the rest is deferred.** The emitter
   DOES infer `ts_module`/`edge_function` → Supabase `table`/`edge_function` (via `supabase.from` /
   `.functions.invoke`, Phase 1a) and → external API / Supabase REST (via bare global `fetch()`,
   follow-up #1). NOT yet inferred: a `this.supabase.from()` DI-pattern call (receiver roots at `this`,
   skipped — a conservative miss, not a false edge; follow-up #2); an import-aliased HTTP client
   (`node-fetch` / `undici` / `axios`); a code → n8n `workflow` webhook call. The substrate can hold
   these; deferred, not dropped.
