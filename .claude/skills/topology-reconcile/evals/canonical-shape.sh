...
use_scratch "$DA"
NODES_A="[ $(mkwf "repo:wf-a" "ID_A" 33 "abc123+dirty"), $(mkwf "cloud:ID_A" null 34 "live:<your-instance>:ID_A") ]"
...
use_scratch "$DB"
NODES_B="[ $(mkwf "repo:wf-b" "ID_B" 33 "abc+dirty"), $(mkwf "cloud:ID_B" null 33 "live:<your-instance>:ID_B") ]"
...
use_scratch "$DD"
NODES_D="[ $(mkwf "repo:wf-d1" "ID_D" 30 "abc+dirty"), $(mkwf "repo:wf-d2" "ID_D" 31 "abc+dirty"), $(mkwf "cloud:ID_D" null 30 "live:<your-instance>:ID_D") ]"
...
use_scratch "$DH"
NODES_H="[ $(mkwf "repo:wf-h" "ID_H" 33 "abc+dirty"), $(mkwf "cloud:ID_H" null 34 "live:<your-instance>:ID_H"), $(mknode "h1" "ts_module" "dependency_cruiser" '{}' "abc+dirty") ]"
...
# count bumped 3 -> 4 when the M6 conservation invariant was registered (intent-conservation-rls.json). This
# is a legitimate registry growth, NOT a masked regression: the M4 versioning+derivation assertions above
# still hold in the SAME pass, and the conservation invariant is asserted to compute independently (it returns
# unverifiable_dimension here — supabase-live is declared-missing in a fresh scratch — a clean non-drift).
echo "$JH" | jq -e '(.invariants | length) == 4 and ([.invariants[] | select(.kind=="conservation")] | length) == 1 and [.invariants[] | select(.kind=="conservation")][0].verdict == "unverifiable_dimension"' >/dev/null || fail "H: not 4 invariants OR conservation did not compute cleanly"; ok "H all 4 invariants computed independently (3 M4 + conservation; no .-scope cross-contamination — S4)"
rm -rf "$DH"

# ---- CASE I — read-only: substrate byte-untouched after a reconcile run ------------------------------
DI="$(setup)" || fail "I setup"
use_scratch "$DI"
NODES_I="[ $(mkwf "repo:wf-i" "ID_I" 33 "abc+dirty"), $(mkwf "cloud:ID_I" null 34 "live:<your-instance>:ID_I") ]"
...
use_scratch "$DJ"
NODES_J="[ $(mkwf "repo:wf-j" "ID_J" 33 "abc+dirty"), $(mkwf "cloud:ID_J" null 34 "live:<your-instance>:ID_J"), $(mknode "j1" "ts_module" "dependency_cruiser" '{}' "abc+dirty") ]"
...
CLOUD_Q="$(jq -nc --arg now "$NOW" '{id:"cloud:ID_Q", kind:"workflow", source_file:"f", source_commit:"live:<your-instance>:ID_Q", timestamp:$now, source_line:null, emitter:"n8n_parser", depends_on:[], depended_on_by:[], attributes:{name:"wf", active:true, n8n_id:null}, declared_intent_ref:null}')"
...
CYC1="$(jq -nc --arg now "$NOW" '{id:"cloud:ID_K", kind:"workflow", source_file:"fixture", source_commit:"live:<your-instance>:ID_K",
...
use_scratch "$DL"
NODES_L="[ $(mkwf "repo:wf-l" "ID_L" 33 "abc+dirty"), $(mkwf "cloud:ID_L" null 34 "live:<your-instance>:ID_L") ]"
