...
#   STEP 1  init scratch.
#   STEP 2  seed the REPO workflow nodes from the live BuyBox-AI substrate (the left_view, real).
...
#   STEP 7  isolation: the REAL BuyBox-AI substrate is byte-untouched throughout.
#
...
# isolation guard: capture the REAL BuyBox-AI substrate state (if present) to assert it is untouched.
...
# ---- STEP 1: init scratch -----------------------------------------------------------------------------
bash "$SUB" init "BuyBox-AI-testB" >/dev/null 2>&1 || fail "STEP 1 init"
ok "STEP 1 scratch substrate init'd (isolated from the real BuyBox-AI substrate)"

# ---- STEP 2: seed the REPO workflow nodes (the left_view) ---------------------------------------------
# Prefer the live BuyBox-AI repo nodes; fall back to a committed fixture if the live substrate is absent.
LIVE_RAN=0
if [ -f "$LIVE_SUBSTRATE" ]; then
...
jq -n --arg now "$NOW" '
  [{ id:"Gz8EKN9CWxIDcGXcoCmYq", name:"Gmail -> Clozers Comps", active:true, isArchived:false,
     ...
     body:{ nodes: ([range(0;33) | {id:("n"+(.|tostring)), name:("Node "+(.|tostring)), type:"n8n-nodes-base.code", position:[0,0], disabled:false}]
            ...
            connections:{} } }]' > "$WF_BASE"
jq -n --slurpfile workflows "$WF_BASE" --arg now "$NOW" --arg src_commit_prefix "live:honeybird:" \
...
bash "$N8N_EMIT" "$SCRATCH/nodes-base.json" "$SCRATCH/edges-base.json" "BuyBox-AI-testB" >/dev/null 2>&1 || fail "STEP 3 frozen emitter"
...
jq -n --slurpfile workflows "$WF_DRIFT" --arg now "$NOW" --arg src_commit_prefix "live:honeybird:" \
...
bash "$N8N_EMIT" "$SCRATCH/nodes-drift.json" "$SCRATCH/edges-drift.json" "BuyBox-AI-testB" >/dev/null 2>&1 || fail "STEP 5 frozen emitter (drift)"
...
# ---- STEP 7: isolation — the real BuyBox-AI substrate is byte-untouched -------------------------------
if [ -f "$LIVE_SUBSTRATE" ]; then
  ...
  ok "STEP 7 real BuyBox-AI substrate byte-untouched (isolation honoured — read-only)"
else
...
if [ "$LIVE_RAN" -eq 1 ]; then
  echo "=== TEST-B-LIVE — all $PASS_COUNT assertions green (repo left_view from the live BuyBox-AI substrate) ==="
else
  echo "=== FIXTURE PROOF — all $PASS_COUNT assertions green (no live BuyBox-AI substrate; repo left_view from fixture) ==="
fi
