...
- **Live Test B proof (2026-06-02)** — DESTINATION Test B (a deliberate drift detected + ranked +
  action-attached, re-derived from real sources): on a scratch substrate seeded with the live BuyBox-AI
  repo workflow nodes + the cloud node re-derived via the FROZEN `n8n-cloud-emitter` against the live
  honeybird workflow `Gz8EKN9CWxIDcGXcoCmYq`. **Baseline** (faithful published graph, 33 cloud == 33 repo)
  ...
  live BuyBox-AI substrate was byte-untouched throughout (read-only honoured).
