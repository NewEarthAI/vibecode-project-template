...
provenances: cloud nodes are `source_commit: "live:<your-instance>:<id>"`, in-repo nodes can be
`source_commit: "<sha>+dirty"`. `git log "live:<your-instance>:..."` errors; a `+dirty` marker is not a clean
ref. Therefore:
...
is incomplete at the entity (the current my-project substrate IS partial — supabase + n8n declared-missing
