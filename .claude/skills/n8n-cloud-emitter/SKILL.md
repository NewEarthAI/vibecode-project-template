...
description: |
  The n8n cloud topology emitter — the second M3 mapper. Reads your project's
  live n8n cloud instance (via the entity's n8n MCP, e.g. mcp__n8n-mcp-<your-instance> for
  my-project) READ-ONLY, transforms the result into Doctrine 05 §6.1 canonical-shape
...
A Claude Code skill that reads your project's **live n8n cloud workflows**, scoped
...
| `workflow.archived` | M4 reconciliation must distinguish graveyard from live (operator's archived workflows are still your project-folder surface, NOT gone) |
...
through the n8n MCP (Claude calls `mcp__n8n-mcp-<your-instance>__*`, parses JSON results, runs
...
1. **Init the substrate** (idempotent — the harness does this itself):
   ```bash
   bash .claude/skills/topology-substrate/scripts/substrate.sh init "<entity, e.g. my-project>"
   ```
2. **Health-check the n8n MCP** (Phase 0 — session-driven, NOT inside emit.sh):
   ```
   mcp__n8n-mcp-<your-instance>__n8n_health_check(mode="status")
   ```
...
> **One `bulk-write`, not a `write-node` loop.** At my-project v1 scale (11 workflows,
...
source_commit: "live:<your-instance>:<workflow_id>"
...
`live:<your-instance>:<workflow_id>` form is deterministic per workflow so a multi-workflow
...
| Archived workflows | EMITTED with `attributes.archived: true` IF in scope | Archived your project-folder workflows are still your project surface (operator's contract). Archived non-your project workflows are out-of-scope by definition (no your project tag, not in id-override) |
...
The frontmatter's `allowed-tools` list names `mcp__n8n-mcp-<your-instance>__*` — my-project's
instance. When `/push-to-template` propagates this skill and another entity (your project,
your project) runs `/update-latest`, the operator must:

1. Update `allowed-tools` in the SKILL.md frontmatter to the receiving entity's MCP
   (e.g. `mcp__n8n-mcp-your-project__*` for your project).
2. Update 📄 `references/entity-scope.json`:
   - `entity` → the entity name (e.g. "your project")
   - `mcp_server` → the MCP server name (e.g. "mcp__n8n-mcp-your-project")
   - `tags` → the per-entity tag set (each project has its own folder/tag
...
3. Update the `--arg src_commit_prefix` value when invoking transform.jq to
   `"live:<entity-instance-slug>:"` (e.g. `"live:your-project-cloud:"` — whatever the
...
**your project has no n8n** (uses Make.com only); on your project, the emitter should NOT be run —
...
2 MB. At my-project v1 (11 workflows, ~250 workflow_nodes, ~400 edges) the two argv
...
1. **Cross-workflow target out-of-scope SKIPPED, not stubbed.** A your project workflow that
   calls a non-your project workflow produces a `calls` edge whose target is not in our
...
| `mcp__n8n-mcp-<your-instance>__*` (my-project) | Read-only workflow queries (`n8n_health_check`, `n8n_list_workflows`, `n8n_get_workflow`). The emitter's allowed-tools list MUST be tightened per-entity (swap to `mcp__n8n-mcp-<entity>__*` at propagation). |
