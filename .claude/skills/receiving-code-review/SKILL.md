...
description: |
  Behavioral protocol for handling received code review feedback. Anti-sycophancy rules,
  YAGNI checks, source-specific handling (human vs external). Use AFTER master-code-reviewer
  produces its review. For conducting reviews, use master-code-reviewer.
  For dispatching reviews, use requesting-code-review.
version: 1.1
source: obra/superpowers (enhanced for the project)
classification: encoded-preference
triggers:
  - "how do I handle this feedback?"
  - "received code review"
  - "implement review suggestions"
do-not-trigger:
  - "review this code" → use master-code-reviewer
  - "request a review" → use requesting-code-review
---

# Code Review Reception

> Behavioral protocol for AFTER a review is received. This is the companion to `master-code-reviewer` (which produces reviews) and `requesting-code-review` (which dispatches them).
...
BAD: "Great point!"
BAD: ANY gratitude expression — actions speak, just fix it.
```
