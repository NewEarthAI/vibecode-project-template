...
2b. Destination nudge — CHECKPOINT 1 companion (advisory; NEVER auto-invoke, NEVER block).
    Where step 2a is the DIAGNOSTIC framing audit ("is the goal the right question?"), this
    is its GENERATIVE companion ("does the goal have a written, falsifiable definition of
    done?"). Planned mode is human-present by construction — it runs in conversation; the
    autofire path is structurally separate and UNATTENDED. So fire this nudge at this single
    point ONLY, never in the autofire path.
    - Gate (one runtime check): a `DESTINATION.md` is ABSENT at the repo root. Planned mode is
      already substantial-by-construction, so no "is this multi-session?" check is needed. If
      `DESTINATION.md` EXISTS, stay silent — `define-destination` refuses silent overwrite and
      the existing destination is already inherited by the continuation.
    - Action: surface a ONE-LINE nudge to the operator — "no DESTINATION.md for this arc —
      consider `Skill define-destination` to author a falsifiable success target that the
      autofired sessions will inherit." Do NOT auto-invoke the skill: it is INTERACTIVE
      (refuses vague input, walks the operator through the recipe), so auto-invoking it inside
      an orchestration that may run unattended risks a hang. A nudge is pure context-injection —
      it cannot hang and cannot block.
    - The operator decides: run `Skill define-destination` (its own three-way scope gate
      self-excludes tactical / open-ended goals; it refuses to overwrite or copy cross-repo)
      OR skip. A skip is a STATED, recorded gap (note it in the autovibe session log) — never
      a silent one. Either way, proceed to step 3.
    - NEVER a ship-blocker: this step does NOT halt the loop and is NOT a required artefact in
      the composition-chain gate. A missing `DESTINATION.md` never refuses the continuation.
    - Why here: a destination authored once at this human-present checkpoint is inherited by
      every autofired session via the master continuation, which cites `DESTINATION.md` by
      pointer (not a copy). Cite `.claude/rules/framing-audit-mandate.md` (the generative-
      complement section) — the destination is the generative counterpart to step 2a's
      diagnostic audit; the two are deliberately paired.
