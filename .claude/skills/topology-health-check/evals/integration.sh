...
# my-project substrate.
#
# Council amendments (council/sessions/2026-06-01-m3-session-6-health-check-integration.md):
#   #7  ALSO assert against the REAL 1,076-node my-project substrate (a structured dependency query),
...
# Capture the REAL my-project substrate's pre-test state to assert it is untouched after the test.
# ---------------------------------------------------------------------------------
...
bash "$SUB" init "my-project-integration" >/dev/null 2>&1 || fail "init"
...
RC_OUT="$(TOPOLOGY_SUBSTRATE_PATH="$TOPOLOGY_SUBSTRATE_PATH" bash "$RC_EMIT" "$RC_NODES_F" "$RC_EDGES_F" "my-project-integration" 2>&1)"; RC_RC=$?
...
# Point the health-check at the REAL my-project substrate (read-only) + assert a Test-A-class structured
...
if [ -f "$LIVE_SUBSTRATE" ]; then
  ...
  echo "  SKIP: live my-project substrate not present at $LIVE_SUBSTRATE — live assertion skipped (run the in-repo emitters there first). The fixture proof (STEP 5) still holds; this is a coverage note, not a pass."
fi
...
# ---------------------------------------------------------------------------------
# STEP 8 — ISOLATION assert (amendment #9): the REAL my-project substrate is byte-unchanged after the test.
# ---------------------------------------------------------------------------------
...
if [ -f "$LIVE_SUBSTRATE" ]; then
  ...
  ok "real my-project substrate unchanged (nodes=$REAL_NODES_AFTER, mtime stable) — isolation honoured"
else
...
if [ "$LIVE_RAN" -eq 1 ]; then
  ...
  echo "    but STEP 6's live-substrate structured query did NOT run (no substrate at the my-project path)."
