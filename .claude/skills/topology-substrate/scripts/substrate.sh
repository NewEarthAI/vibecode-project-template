...
# kind = the D05 §6.1 domain kinds (config added M3 Session 5 for declared-structure config
# files — vercel.json / package.json — via the doctrine-verification-gate triple gate; record:
# council/code-reviews/2026-06-01-m3-session-5-repo-config-emitter.md). `external_endpoint` added
# 2026-06-05 by the visual-layer schema delta (cross-system edges + blind-spot honesty marker) via
# the same triple gate; record: council/code-reviews/2026-06-05-visual-layer-schema-delta.md. It is
# the target/marker for a cross-system dependency edge (frontend→table/edge-fn, n8n→REST, edge-fn→
# external API); a RUNTIME-CONSTRUCTED/unresolvable target is a `blind-spot` external_endpoint
# (D05 §3.5 — never dropped, never green). `manual` is an EMITTER value (the source-orphan marker,
# D05 §6.6), NOT a kind — a manual node still carries one of the domain kinds (a config node is
# always emitter:manual + manual_justification).
VALID_KINDS="table view function trigger rls_policy edge_function workflow workflow_node ts_module config external_endpoint"
# external_api_parser added 2026-06-05 (visual-layer delta): owns external_endpoint nodes emitted by
# the external-api-graph emitter (Phase 1b). NOTE the documented joint-attribution exception
# (canonical-shape.md): the code emitter (dependency_cruiser) may ALSO emit `blind-spot`
# external_endpoint nodes for unresolved frontend targets — one node still has exactly one emitter
# (D05 P4); the kind has two legitimate producers, recorded to keep owned_but_uncovered honest.
VALID_EMITTERS="pg_depend n8n_parser dependency_cruiser vercel_api manual external_api_parser"
