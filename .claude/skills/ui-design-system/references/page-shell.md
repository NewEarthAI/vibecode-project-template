...
```
┌─────────────────────────────────────────────────────┐
│ [← Back]   ideas › team – AZ     [Pause][Dup][×] │  ← STICKY, z-40
│                                                      │     (back + breadcrumb +
│                                                      │      actions ALWAYS
│                                                      │      rendered here)
├─────────────────────────────────────────────────────┤
...
<PageShell
  backTo="/ideas"                         // string path, or number (history delta)
  backLabel="ideas"                       // optional; falls back to breadcrumb
  breadcrumbs={[
...
Before this skill existed, the SaaS app `ListManager` + `ItemDetail` pages each had:
- Back button at `mb-4` above the content (outside a container)
- Header row at `mb-8` with its own title/actions layout
- `ListManager` additionally had a "Back to Dashboard" ghost button at the *bottom* of the page — 600px below the primary action
- No sticky behavior — scrolling past the "New idea" button meant the user had to scroll all the way up to create another one

User feedback (2026-04-19): *"They can't go back or up easily, or click New idea or Import. There's not even a back button at the top when there should be."*
