...
|-----------|------------|
| `primary_color` | guided-tour tour button + ui-design primary button variant |
| `primary_color_foreground` | ui-design button text on primary |
| `font_family` | guided-tour tooltip copy + ui-design body text — **same parameter name in both skills for theming alignment** |
| `font_family_mono` | ui-design-only — for numeric data, IDs, code. guided-tour does not need a mono font, so this parameter is exclusive to ui-design-system. |
...
| Internal dashboard, data-dense interface, CRUD app | `ui-design-system` (house style) |
| Client proposal site, branded microsite | Both — frontend-design for composition, ui-design-system for token precision |
| Component library for a new client | Both — frontend-design for initial tone, ui-design-system for system lockdown |
...
> *"See `frontend-design` plugin skill for the general AI-cliche ban list (purple gradients, generic Inter, cookie-cutter hero layouts). This file adds the the agency-specific hard rules on top."*
...
- **Re-skins** the shadcn-ui base components with the agency tokens
...
Every the agency project inherits from the house style but parameterizes one thing: the **primary brand color**.
...
- Neutral palette (always the agency warm off-white stack)
- Silver signature (always the agency silver)
...
**Why**: these are what make every the agency project recognizably the agency. They are the *signature*. If they varied per client, the agency would have no house style — just a collection of disconnected projects.
...
bash .claude/skills/ui-design-system/scripts/audit-forbidden-patterns.sh ./
bash .claude/skills/ui-design-system/scripts/audit-colors.sh ./
bash .claude/skills/ui-design-system/scripts/audit-hover-consistency.sh ./
```
...
User: "Set up a new client project called Acme Logistics with brand color #EA580C"
  → ui-design-system handles tokens, components, layout
...
User: "Audit my dashboard for design quality"
  → ui-design-system runs the three audit scripts
...
User: "Add a revenue KPI card to the dashboard"
  → ui-design-system applies Recipe 2 (KPI Hero Card)
...
User: "Create theme tokens for this client — brand is teal #14B8A6"
  → ui-design-system copies assets/tokens.css
...
*This skill sits at the top of the stack. It inherits bold direction from frontend-design, infrastructure from guided-tour, and primitives from shadcn-ui — and locks them all together with the agency house style.*
