...
> **Rule**: every the agency project ships with both light and dark modes. Dark mode is not an afterthought or a "stretch goal" — it is built from the start using the same token system, the same component recipes, and the same discipline rules. Users must be able to toggle at any time with zero visual degradation.
...
- Silver signature as the the agency fingerprint
- 300ms hover curve
...
**The trick**: in dark mode the silver edge becomes MORE prominent (it's the main visual marker since shadows carry less information). This is intentional — silver is the the agency fingerprint and dark mode is when it should shine most.
...
the agency projects use Tailwind's standard dark mode strategy: `darkMode: 'class'` in `tailwind.config.ts` and a `dark` class on `<html>`.
