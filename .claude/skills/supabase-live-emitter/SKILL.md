...
> **One `bulk-write`, not a `write-node` loop.** At my-project scale (~1,143 nodes), a
...
| Filter | Rule | Reason |
|---|---|---|
| Schemas | `public`, `dd_clean`, `dd_v3` (my-project's user schemas) | The goal is the project's OWN topology, not the Supabase platform. `pg_catalog`, `information_schema`, `auth`, `storage`, `realtime`, `extensions`, `cron`, `pgbouncer`, `pgsodium`, `vault`, `graphql*`, `net`, `supabase_*` are platform/extension surface — out of scope. (For other projects, pass the schema list at invocation.) |
| Functions | `prokind='f'` AND NOT extension-owned (`pg_depend.deptype='e'`) | Skips aggregates (`prokind='a'`) and window funcs (`prokind='w'`) that came from PostGIS / pgcrypto / etc. The 801 extension-owned + 23 aggregate + 2 window functions in my-project's `public` are platform noise, not user code. |
...
my-project's MCP. When `/push-to-template` propagates this skill and another entity (your project,
...
1. Update `allowed-tools` in the SKILL.md frontmatter to the receiving entity's MCP
   (e.g. `mcp__supabase-your-project__execute_sql` for your project).
...
MB. At my-project today (~1,143 nodes, ~610 edges) the two argv strings total roughly
...
1. **plpgsql function→table edges are SPARSE.** Postgres's `pg_depend` does not record
   ...
   pg_depend traces). my-project's functions are dominantly `plpgsql` (per the Q3 sample), so
...
5. **Function overloading collapses to one node.** Postgres allows two functions with the same
   ...
   collapses them — `bulk-write` keeps the last-write-wins record. Observed at my-project:
