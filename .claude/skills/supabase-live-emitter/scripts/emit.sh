#!/usr/bin/env bash
# supabase-live-emitter/scripts/emit.sh — the finishing harness for the pg_depend emitter.
#
# Intent-Actual-Gap Mechanism Build Programme, M3 Session 2. Takes pre-collected JSON inputs
# (the invoking Claude session runs the catalogue queries via the Supabase MCP and writes
# the canonical-shape nodes/edges arrays to two files), then atomically:
...
#   4. calls mark-emitter-ran supabase-live covered
...
#   [entity]            optional; defaults to env var SUPABASE_LIVE_ENTITY or 'your-entity'
...
NODES_FILE="${1:-}"
EDGES_FILE="${2:-}"
ENTITY="${3:-${SUPABASE_LIVE_ENTITY:-your-entity}}"
...
# first attempt left partial state (CRITICAL finding from code-council review).
...
# (~1,143 nodes => ~250-500KB of JSON, well under ARG_MAX of ~1MB on macOS).
...
# --- mark-emitter-ran -----------------------------------------------------------
if ! bash "$SUBSTRATE_SH" mark-emitter-ran supabase-live covered; then
  echo "emit.sh: mark-emitter-ran supabase-live covered failed — substrate write succeeded but heartbeat did NOT register (Reliability-Engineer NON-SHIPPABLE state — investigate immediately)" >&2
  exit 6
fi
...
# This matches the eval's anchored discipline (canonical-shape.sh) — code-council CRITICAL.
...
echo "emit.sh: PASS — emitted $NODE_COUNT nodes, $EDGE_COUNT edges; supabase-live coverage=covered"
exit 0
