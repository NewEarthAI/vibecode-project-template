#!/usr/bin/env bash
# malformed-rejected.sh — negative eval for the supabase-live emitter.
#
...
SCRATCH="$(mktemp -d -t supabase-live-neg-eval-XXXXXX)"
trap 'rm -rf "$SCRATCH"' EXIT
export TOPOLOGY_SUBSTRATE_PATH="$SCRATCH/topology-graph.json"

NOW="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
SRC_COMMIT="live:fixture:postgres"
...
# closing the negative-eval false-positive class the code-council flagged.
assert_rejected() {
...
[{"id":"public.good_table","kind":"table","source_file":"pg_catalog (live)","source_commit":"$SRC_COMMIT","timestamp":"$NOW","source_line":null,"emitter":"pg_depend","depends_on":[],"depended_on_by":[],"attributes":{"columns":["id"],"row_estimate":1},"declared_intent_ref":null}]
EOF
...
# false-positive class the code-council flagged.
echo '[]' > "$SCRATCH/empty-edges.json"

# Scenario 1: node missing required kind ----------------------------------
cat > "$SCRATCH/bad1-nodes.json" <<EOF
[{"id":"public.bad1","source_file":"pg_catalog (live)","source_commit":"$SRC_COMMIT","timestamp":"$NOW","source_line":null,"emitter":"pg_depend","depends_on":[],"depended_on_by":[],"attributes":{},"declared_intent_ref":null}]
EOF
...
[{"id":"public.lonely","kind":"table","source_file":"pg_catalog (live)","source_commit":"$SRC_COMMIT","timestamp":"$NOW","source_line":null,"emitter":"pg_depend","depends_on":[],"depended_on_by":[],"attributes":{"columns":["id"],"row_estimate":0},"declared_intent_ref":null}]
EOF
cat > "$SCRATCH/bad2-edges.json" <<'EOF'
[{"source":"public.lonely","target":"public.does_not_exist","type":"depends_on","direction":"forward","weight":1}]
EOF
...
[{"id":"public.bad3","kind":"NOT_A_KIND","source_file":"pg_catalog (live)","source_commit":"$SRC_COMMIT","timestamp":"$NOW","source_line":null,"emitter":"pg_depend","depends_on":[],"depended_on_by":[],"attributes":{},"declared_intent_ref":null}]
EOF
...
# The original good_table node must still be the only node.
