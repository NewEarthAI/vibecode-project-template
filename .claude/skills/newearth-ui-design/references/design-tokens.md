...
> **This file is the source of truth.** Every value here is LOCKED across all NewEarth AI projects except where marked `{{parameter}}`. When refining shades later, edit this file and `tokens.css` together.
...
## 2. Silver Signature Tokens (NewEarth Fingerprint) — LOCKED
...
| Tailwind Default | NewEarth Value | Difference |
|-----------------|---------------|------------|
...
**Never deviate**. This curve is the NewEarth house signature hover. Different cards get different *content*, never a different *hover curve*.
