---
name: newearth-ui-design
description: |
  NewEarth AI's house UI/UX design system for React/Vite/Tailwind dashboards and web apps.
  ...
  "does this look vibe-coded". Enforces the NewEarth signature: monochrome-first palette with
...
user-invocable: true
note: Human entry point for BUILDING in the NewEarth house style. Composes the L1/L2/L3 stack under the hood. Sibling entry point is /design-review (audit).
created: 2026-04-10
updated: 2026-04-26
validated_on:
...
triggers:
  ...
  - "apply NewEarth aesthetic" / "use house style" / "NewEarth design"
...
do-not-trigger:
  - "build a generic Tailwind component without NewEarth context" → use tailwind-shadcn-system
...
# NewEarth AI UI/UX Design Skill
...
This skill is the **L3 house-tokens layer** of the NewEarth Design Suite v2 three-layer stack. It does NOT generate output in isolation. Before producing any code, mockup, audit, or recommendation, you MUST read the upstream contract files IN FULL:

1. **`PRODUCT.md`** at the repo root — the NewEarth identity contract (register, brand tokens, audience, tone).
...
```
ERROR (newearth-ui-design): L1/L2 contract files missing.
...
| **Silver as signature** | NewEarth AI fingerprint — hairline metallic borders on premium cards + universal silver hover ring. |
...
4. Do NOT change neutrals, semantic colors, or silver values — those are NewEarth house
...
| [references/silver-signature.md](references/silver-signature.md) | When applying the NewEarth metallic accent system |
...
5. **No `Inter` / `Roboto` / `Arial` / `system-ui` as primary typeface** — DM Sans is the NewEarth default. Client-specific overrides must be a licensed premium typeface, never a system font. Atelier Dark preset uses PP Editorial New + Söhne + Berkeley Mono (or their free equivalents).
...
*Skill version 1.0 — NewEarth AI house design system. Push to template via `/template-push` when validated.*
