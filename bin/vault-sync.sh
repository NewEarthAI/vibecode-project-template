...
# Runs every 10 minutes via launchd (see bin/launchd/com.newearthai.vault-sync.plist).
#
...
SUPABASE_URL="${SUPABASE_URL:?SUPABASE_URL env var required — set to https://<ref>.supabase.co. Prod (newearthai-prod) is ridqdojzjotlvexfuwvx; staging is iqgsthdhkbkjpdqnfisu. Launchd plist (~/Library/LaunchAgents/com.newearthai.vault-sync.plist) MUST set this in EnvironmentVariables; activate-vault-autopilot.sh will fail-loud if missing.}"
...
SERVICE_KEY=$(security find-generic-password -s "agency-supabase-newearthai-service-role-jwt" -a "service_role" -w 2>/dev/null)
[ -n "$SERVICE_KEY" ] || fail "service_role JWT missing from Keychain — see agency/memory/reference_supabase-newearthai-credentials.md"
