...
v0.1.2 (2026-03-03): Single event= API, combinator parsing, cleaner error handling.
"""
...
class Condition:
    ...
    operator: str  # "regex_match", "regex", "contains", "equals", etc.
...
class Rule:
    ...
    name: str
    enabled: bool
    event: str  # "PreToolUse", "PostToolUse", "SessionStart", "Stop", etc.
    pattern: Optional[str] = None  # Simple pattern (legacy)
    conditions: List[Condition] = field(default_factory=list)
    action: str = "warn"  # "warn", "block", or "addContext"
    tool_matcher: Optional[str] = None  # Tool name pattern (supports globs)
    message: str = ""  # Message body from markdown
    combinator: str = "and"  # "and" or "or" for conditions
    ...
    def from_dict(cls, frontmatter: Dict[str, Any], message: str) -> 'Rule':
        ...
        # New style: explicit conditions (list or dict with combinator)
        if 'conditions' in frontmatter:
            cond_data = frontmatter['conditions']

            if isinstance(cond_data, dict):
                # Dict style: {combinator: "or", conditions: [...]}
                combinator = cond_data.get('combinator', 'and')
                cond_list = cond_data.get('conditions', [])
                if isinstance(cond_list, list):
                    conditions = [Condition.from_dict(c) for c in cond_list
                                  if isinstance(c, dict)]
            elif isinstance(cond_list := cond_data, list):
                conditions = [Condition.from_dict(c) for c in cond_list
                              if isinstance(c, dict)]

        # Check for combinator parsed as sibling key
        # (simple YAML parser may extract it separately)
        if 'conditions_combinator' in frontmatter:
            combinator = frontmatter['conditions_combinator']

        # Legacy style: simple pattern field
        simple_pattern = frontmatter.get('pattern')
        if simple_pattern and not conditions:
            event = frontmatter.get('event', 'all')
            if event in ('bash', 'PreToolUse'):
...
def extract_frontmatter(content: str) -> tuple[Dict[str, Any], str]:
    ...
    for line in lines:
        ...
        # Metadata key within a list context (e.g., combinator: or)
        # These are at indent 2, not list items, within a list-valued key
        elif indent >= 2 and in_list and ':' in stripped and not stripped.startswith('-') and not in_dict_item:
            k, v = stripped.split(':', 1)
            k = k.strip()
            v = v.strip().strip('"').strip("'")
            if k == 'combinator' and current_key:
                frontmatter[current_key + '_combinator'] = v
            elif k == 'conditions' and not v:
                # Nested 'conditions:' key — items below belong to outer list
                pass
...
def load_rules(event: Optional[str] = None) -> List[Rule]:
    ...
    Args:
        event: Optional event filter ("PreToolUse", "PostToolUse", "Stop", etc.)
