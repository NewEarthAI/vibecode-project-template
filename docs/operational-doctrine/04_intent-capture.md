...
binary_test         # the third-party-observable success test (positive acceptance proof)
falsifier           # the machine-executable anti-acceptance condition (M6 addition — §6.1.1)
...
The first five fields (`id`, `kind`, `source_file`, `source_commit`, `timestamp`) are the **provenance envelope** shared with the other two mechanisms (Doctrines 05, 06). The remaining nine are the intent payload — the authored-promise fields that distinguish this mechanism (see Section 12, the Test D defence, for why this distinction is load-bearing). The schema is **14 fields** as of the M6 build (the `falsifier` field was added; the schema-review re-run is logged in §12.1.1).

### 6.1.1 The `falsifier` field — machine-executable anti-acceptance (M6 addition)

`binary_test` proves the promise is *met* ("what must be true for this to be kept?"). `falsifier` names the condition under which a *passing* `binary_test` is **misleading** ("what would make a green result a lie?"). The two are complementary, never redundant.

The field is governed by three rules an M6 mechanism enforces at index time (the NSF-2 validator):

1. **Machine-executable form, never prose.** The falsifier is a runnable predicate — a bash/jq expression, a referenced shell script path, or a structured assertion — that a machine can evaluate to true/false without a human reading prose. A prose falsifier (e.g. "make sure the data isn't inflated by coupon traffic") is **rejected** at index time (`falsifier_format_invalid`). This is the direct guard against the vacuous-green-light failure: a `DESTINATION.md` whose conditions are prose must not pass as "real intent captured."
2. **Referential integrity.** Any path, function, or script the falsifier references must resolve. An unresolvable reference is flagged `falsifier_broken_pointer` (distinct from a format violation) — a falsifier pointing at a deleted check is as dangerous as no falsifier, because it looks valid but never runs.
3. **Complementarity, not negation.** The falsifier must evaluate *differently* from `binary_test` on at least one input — it must catch a failure mode `binary_test` does not. A falsifier that is the mechanical negation of `binary_test` adds nothing (it collapses the pair to one guard) and is rejected. The canonical example: `binary_test` = "the RLS policy node exists"; `falsifier` = "the RLS policy node exists but `enabled:false`" — the present-but-inert security control, which a node-existence `binary_test` passes while the promise is violated.

**Falsification**: if a `falsifier` field's value is prose, OR references an unresolvable target, OR is the pure negation of `binary_test`, the field has degraded to documentation and the present-but-inert failure class (the disabled-but-present security control, Doctrine 06 A.6) goes invisible.
...
#### 12.1.1 Test D re-run log (A5 living invariant)

Per the A5 trigger (§12.2): every schema-touching M3+ change appends a dated row here.

| Date | Change | Schemas touched | Re-run result | <50%? |
|------|--------|-----------------|---------------|-------|
| 2026-06-05 | **Visual-layer schema delta** (downstream app #1): new `kind` value `external_endpoint`; new `emitter` value `external_api_parser`; new per-kind `attributes` family (`url_host`/`url_path_template`/`method`/`classification`); new additive nullable `edge.attributes` `{derivation, confidence}`. | **Topology only** | **No node-schema field added** to any of the three mechanisms. `external_endpoint`/`external_api_parser` are *values* of the existing `kind`/`emitter` fields, not new fields. `edge.attributes` is an **edge** field; neither the intent record schema (§6.1) nor the reconciliation record schema (D06 §6.1) carries edge attributes, so it adds zero shared field. Intent∩Topology stays **27.8%** (5/18), Intent∩Reconciliation stays **21.7%** (5/23), Topology∩Reconciliation stays **22.7%** (per `council/audits/2026-05-22-cross-doctrine-consistency-04-05-06.md`). The delta **strengthens** separability (topology gains payload the others lack), exactly as the M3-Session-5 `config`-kind re-check concluded. | yes (all three) |
| 2026-06-07 | **M6 intent-capture build — `falsifier` field added** to the Intent schema (§6.1, §6.1.1). Intent schema 13 → **14 fields** (9 payload + 5 provenance envelope). | **Intent only** | The `falsifier` field is intent-only payload — it grows the Intent side of each union without touching the 5-field provenance intersection. **Intent∩Topology = 5/(14+10−5) = 5/19 = 26.3%**; **Intent∩Reconciliation = 5/(14+17−5) = 5/26 = 19.2%**. Both moved FURTHER below 50% vs the 2026-05-22 snapshot (27.8%→26.3%, 21.7%→19.2%) — separability **strengthened**. Topology∩Reconciliation unchanged (22.7%; M6 touches neither). Per `council/sessions/2026-06-07-m6-intent-capture-plan-v2-execution-soundness.md` (Neutral Analyst 97%). | yes (all three) |
| 2026-06-07 | **M6 — `left_source` field added to the RECONCILIATION schema** (D06 §6.1 — the conservation class's two-source selector). Reconciliation schema 17 → **18 fields**. | **Reconciliation only** | The `left_source` field is reconciliation-only payload (selects whether the conservation class's left_view comes from the intent store vs topology) — it grows the Reconciliation side of each union without touching the provenance intersection. **Reconciliation∩Intent = 5/(18+14−5) = 5/27 = 18.5%**; **Reconciliation∩Topology = 5/(18+10−5) = 5/23 = 21.7%**. Both moved further below 50% — separability **strengthened**, same logic as the M4 A9/A10 additions. Logged here AND in D06 §12.1 (the reconciliation doctrine's own log). | yes (all three) |
...
**Situation**: my-project's destination promises "the property list page loads in under 2 seconds."

**Intent record**: `kind: destination`; `conditions` = "list page TTFB < 2s"; `binary_test` = "a third party times the cold-cache list page and observes < 2s"; `wired_to` = [the list RPC node, the composite index node, the Vercel route node]; `status: accepted`; `owner: your project lead`; `acceptance_cadence: monthly`.
...
**Situation**: your project's operational intent lives largely in Make.com scenario configuration (a closed API, not machine-readable). The repo has a DESTINATION.md and a few ADRs, but the bulk of "what the trip-state machine is supposed to do" is locked in Make.com.
...
**Situation**: an autonomous `/autovibe` session at my-project asks "are all accepted billing-related promises currently fulfilled?" before it begins a billing-adjacent change.
...
**Situation**: my-project exposes a Supabase edge function with a documented request/response contract. The contract IS the intent — it promises a specific input/output shape to any consumer.
...
**Situation**: your project has accumulated ~80 ADRs + 4 destinations + dozens of roadmap items across several bounded contexts. A session asks about one narrow topic ("the KI pipeline relevance-scoring decision").
...
**Situation**: the intent-capture mechanism is propagated from the workshop to your project, whose stack (no n8n, Make.com automation, vendor dispatch app) differs sharply from my-project's.

**Intent record**: at your project, the carriers present are DESTINATION.md + a thin ADR set; the contract-kind and rich roadmap-intent that my-project has are largely absent.
...
**What the doctrine says** (stack-agnosticism, destination Condition 5): the doctrine's record shape and principles are stack-independent — an intent record is an intent record whether the implementation it wires to is a Supabase RPC or a Make.com scenario. The mechanism indexes what is machine-readable, declares-and-logs what is degenerate, and serves the same query interface. The doctrine does not assume my-project's stack.

**Without it**: a stack-specific intent mechanism (one that assumed Supabase + n8n) would silently produce an empty or broken corpus at your project and the propagation would "peel off after the first week of novelty" — the exact failure the destination's Condition 7 (30-day sustain) forbids.

**Composition trace**: the degenerate-case declaration (A.5) propagates to reconciliation at your project, which scopes its invariants to the covered surface. Stack-agnosticism is a property the three mechanisms share precisely because each reads source-of-truth artefacts generically (intent reads any of the four carrier kinds; topology reads any source emitter; reconciliation compares any two views) rather than hard-coding one stack.
